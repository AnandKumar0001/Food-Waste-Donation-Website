...
$mob = mysqli_real_escape_string($conn,($_POST["signup_mobile"]));
  ...
  $check_email = mysqli_num_rows(mysqli_query($conn, "SELECT email FROM agentlogin WHERE email='$email'"));
...
$phone ="+91"."$mob";
  ...
  if($sms_status['Status'] !== 'Success') {
    ...
    $sql = "INSERT INTO agentlogin (fullname, email, password, token, status,mob) VALUES ('$full_name', '$email', '$password', '$otp', '0','$mob')";
    $result = mysqli_query($conn, $sql);
    echo "<script>alert('OTP Sent');</script>";
    echo '<script>window.location="verifyagent.php"</script>';
...
if (isset($_POST["signin"])) {
  ...
  $check_fullname = mysqli_num_rows(mysqli_query($conn, "SELECT fullname FROM agentlogin WHERE fullname='$full_name'"));

  $check_email = mysqli_query($conn, "SELECT id FROM agentlogin WHERE email='$email' AND password='$password' AND status='1'");
   $_SESSION["fullname"] = "$full_name";
if ($check_fullname<1) {
...
<head>
  ...
  <title>Agent Login and Singup</title>
</head>
    ...
    <div class="forms-container">
      <div class="signin-signup">
        <form action="" method="post" class="sign-in-form">
          <h2 class="title">Agent Sign in</h2>
          ...
          <p style="display: flex;justify-content: center;align-items: center;margin-top: 20px;"><a href="forgot-password-agent.php" style="color: #4590ef;">Forgot Password?</a></p>
        </form>
