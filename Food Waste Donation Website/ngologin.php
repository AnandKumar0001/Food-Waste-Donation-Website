...
$_SESSION["fullname"] = "$full_name";
  ...
  if($sms_status['Status'] !== 'Success') {
    ...
    $sql = "INSERT INTO ngo (fullname, email, password, token, status, mob,orgn) VALUES ('$full_name', '$email', '$password', '$otp', '0' ,'$mob','$orgn')";
    $result = mysqli_query($conn, $sql);
    echo '<script>window.location="verifyngo.php"</script>';
...
if (isset($_POST["signin"])) {
  ...
  $check_fullname = mysqli_num_rows(mysqli_query($conn, "SELECT fullname FROM ngo WHERE fullname='$full_name'"));

  $check_email = mysqli_query($conn, "SELECT id FROM ngo WHERE email='$email' AND password='$password' AND status='1'");
   $_SESSION["fullname"] = "$full_name";
if ($check_fullname<1) {
...
<head>
  ...
  <title>Needy Login And Singup</title>
</head>
    ...
    <div class="forms-container">
      <div class="signin-signup">
        <form action="" method="post" class="sign-in-form">
          <h2 class="title">Needy Sign in</h2>
          ...
          <p style="display: flex;justify-content: center;align-items: center;margin-top: 20px;"><a href="forgot-password-ngo.php" style="color: #4590ef;">Forgot Password?</a></p>
        </form>
        <form action="" class="sign-up-form" method="post">
          ...
          <div class="input-field">
            <i class="fas fa-envelope"></i>
            <input type="text" placeholder="Organisation Name" name="signup_orgn" value="<?php echo $_POST["signup_orgn"]; ?>" required  />
          </div>
          <input type="submit" class="btn" name="verify" value="Verify User" />
