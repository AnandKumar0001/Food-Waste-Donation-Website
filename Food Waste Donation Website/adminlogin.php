...
if (isset($_POST["verify"])) {
  ...
  $check_email = mysqli_num_rows(mysqli_query($conn, "SELECT email FROM login WHERE email='$email'"));
  ...
  if($sms_status['Status'] !== 'Success') {
    ...
    $sql = "INSERT INTO login (fullname, email, password, token, status, mob) VALUES ('$full_name', '$email', '$password', '$otp', '0' ,'$mob')";
...
if (isset($_POST["signin"])) {
  ...
  $check_fullname = mysqli_num_rows(mysqli_query($conn, "SELECT fullname FROM login WHERE fullname='$full_name'"));

  $check_email = mysqli_query($conn, "SELECT id FROM login WHERE email='$email' AND password='$password' AND status='1' AND adminr='1'");
   $_SESSION["fullname"] = "$full_name";
if ($check_fullname<1) {
...
<head>
  ...
  <title>Admin Login And Singup</title>
</head>
