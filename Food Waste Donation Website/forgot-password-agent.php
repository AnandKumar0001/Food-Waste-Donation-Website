...
if (isset($_POST["sendotp"])) {
  ...
  $check_mob = mysqli_query($conn, "SELECT * FROM agentlogin WHERE mob='$mob'");
  if (mysqli_num_rows($check_mob) > 0) {
    $data = mysqli_fetch_assoc($check_mob);
     $update = mysqli_query($conn, "UPDATE agentlogin SET token='$otp' WHERE mob='$mob'");
     echo "<script>alert('Otp Sent');</script>";
     echo '<script>window.location="otpform-agent.php"</script>';
